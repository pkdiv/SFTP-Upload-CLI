...
import (
	...
	"github.com/easysftp/sftp-upload/internal/confirmation"
	"github.com/easysftp/sftp-upload/internal/output"
	"github.com/easysftp/sftp-upload/internal/uploader"
)
...
Usage:
  sftp-upload upload <profile> [--concurrency N] <files...>
...
func (a *App) runUpload(args []string) int {
	...
	remaining := fs.Args()
	if len(remaining) < 2 {
		fmt.Fprintln(a.ErrOut, "usage: sftp-upload upload <profile> [--concurrency N] <files...>")
		return 2
	}
...
func (a *App) runProfile(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(a.ErrOut, "usage: sftp-upload profile <add|list|show|edit|remove> [name]")
		return 2
	}
	...
	case "show":
		if len(args) < 2 {
			fmt.Fprintln(a.ErrOut, "usage: sftp-upload profile show <name>")
			return 2
		}
		return a.profileShow(args[1])
	case "edit":
		if len(args) < 2 {
			fmt.Fprintln(a.ErrOut, "usage: sftp-upload profile edit <name>")
			return 2
		}
		return a.profileEdit(args[1])
	case "remove":
		if len(args) < 2 {
			fmt.Fprintln(a.ErrOut, "usage: sftp-upload profile remove <name>")
			return 2
		}
